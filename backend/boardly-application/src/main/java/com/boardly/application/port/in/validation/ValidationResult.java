...
import com.boardly.domain.common.Failure.FieldViolation;
...
public final class ValidationResult<T> {
    ...
    public static <T> ValidationResult<T> invalid(FieldViolation violation) {
        return new ValidationResult<>(
                Validation.invalid(io.vavr.collection.List.of(violation)));
    }
    ...
    public static <T> ValidationResult<T> of(
            Validation<Seq<FieldViolation>, T> validation) {
     ...
     * @return 변환된 결과
     */
    public <U, R> ValidationResult<R> flatMap(
            Function<T, ValidationResult<R>> mapper) {
        if (validation.isInvalid()) {
            return new ValidationResult<>(
                    Validation.invalid(validation.getError()));
        }
    ...
    public <U, R> ValidationResult<R> combine(
        ...
        return ValidationResult.of(
                validation
                        .combine(other.validation)
                        .ap((t, u) -> combiner.apply(t).apply(u))
                        .mapError(errors -> errors.flatMap(e -> e)));
    }

    /**
     * 검증 실패 결과를 Failure로 변환합니다. errorCode를 VALIDATION_ERROR로 고정합니다.
     *
     * @param messageKey 메시지키
     * @return 실패 결과
     * @throws IllegalStateException 검증 성공 시 예외 발생
     */
    public Failure toFailure(String messageKey) {
        if (validation.isValid()) {
            throw new IllegalStateException("ValidationResult is valid");
        }

        return Failure.ofInputError(
                messageKey,
                List.copyOf(getErrorsAsCollection()));
    }
     ...
     * @return 검증 결과
     */
    public ValidationResult<T> peekError(
            Consumer<Seq<FieldViolation>> consumer) {
        if (validation.isInvalid()) {
     ...
     */
    public <U> U fold(
            Function<Seq<FieldViolation>, U> onInvalid,
