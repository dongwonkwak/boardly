...
const PUBLIC_APIS = new Set(["registerUser"]);
	...
	return ((...args: any[]) => {
		...
		if (hasOpts) {
			...
			const mergedOpts = {
				...
				headers: {
					...((lastArg as Record<string, unknown>).headers as Record<
						string,
						unknown
					>),
					...authOpts.headers,
...
export async function updateAuthenticatedUser(
	accessToken: string,
	updateUserRequest: apiClient.UpdateUserRequest,
) {
...
export async function registerNewUser(
	registerUserRequest: apiClient.RegisterUserRequest,
) {
	return (publicApi.registerUser as typeof apiClient.registerUser)(
		registerUserRequest,
	);
}
