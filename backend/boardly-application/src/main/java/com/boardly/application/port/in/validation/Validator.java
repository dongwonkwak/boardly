...
public interface Validator<T> {
    ...
    default <U> Validator<U> contramap(Function<U, T> mapper) {
        return target -> this.validate(mapper.apply(target)).map(result -> target); // 원본 타입으로 다시 변환
    }
    ...
    // 항상 실패
    static <T> Validator<T> invalid(String field, String messageKey, Object... messageArgs) {
        return target -> ValidationResult.invalid(field, messageKey, target, messageArgs);
    }
    ...
    static <T> Validator<T> of(
        ...
        return target -> {
            ...
            return ValidationResult.invalid(field, messageKey, target, messageArgs);
        };
    }
     ...
     */
    static <T> Validator<T> fromValidation(
            Function<T, Validation<Seq<FieldViolation>, T>> validationFunc) {
