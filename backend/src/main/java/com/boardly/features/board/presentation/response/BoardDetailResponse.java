...
public record BoardDetailResponse(
        ...
        public static BoardDetailResponse from(BoardDetailDto boardDetailDto) {
                ...
                List<BoardColumnResponse> columns = boardDetailDto.columns().stream()
                                ...
                                .map(boardList -> {
                                        // 해당 리스트의 카드들 필터링
                                        List<com.boardly.features.card.domain.model.Card> listCards = boardDetailDto
                                                        .cards().stream()
                                                        .filter(card -> card.getListId().equals(boardList.getListId()))
                                                        .sorted((a, b) -> Integer.compare(a.getPosition(),
                                                                        b.getPosition()))
                ...
                List<BoardMemberResponse> boardMembers = boardDetailDto.boardMembers().stream()
                                .map(boardMember -> {
                                        com.boardly.features.user.domain.model.User user = userMap
                                                        .get(boardMember.getUserId().getId());
        ...
        private static BoardCardResponse convertToBoardCardResponse(
                ...
                List<CardAssigneeResponse> assignees = cardMembers.getOrDefault(card.getCardId(), List.of())
                                ...
                                .map(cardMember -> {
                                        com.boardly.features.user.domain.model.User user = userMap
                                                        .get(cardMember.getUserId().getId());
                                        if (user != null) {
