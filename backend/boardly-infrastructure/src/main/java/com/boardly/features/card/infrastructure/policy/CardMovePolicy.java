package com.boardly.features.card.infrastructure.policy;
...
public class CardMovePolicy {
     ...
     */
    public Either<Failure, Void> canMoveWithinSameList(
        Card card,
        int newPosition
    ) {
        log.debug(
            "같은 리스트 내 카드 이동 정책 검증: cardId={}, oldPosition={}, newPosition={}",
     ...
     * @return 성공 시 Right(Void), 실패 시 Left(Failure)
     */
    public Either<Failure, Void> canMoveToAnotherList(
        Card card,
        ListId targetListId,
        int newPosition
    ) {
        log.debug(
            "다른 리스트로 카드 이동 정책 검증: cardId={}, sourceListId={}, targetListId={}, newPosition={}",
            card.getCardId().getId(),
     ...
     */
    private Either<Failure, Void> checkTargetListCardLimit(
        ListId targetListId
    ) {
        long currentCount = cardRepository.countByListId(targetListId);

        log.debug(
            "대상 리스트 카드 개수 확인: targetListId={}, currentCount={}, maxCount={}",
            targetListId.getId(),
            currentCount,
            MAX_CARDS_PER_LIST
        );
        ...
        if (currentCount >= MAX_CARDS_PER_LIST) {
            log.warn(
                "대상 리스트 카드 개수 제한 초과: targetListId={}, currentCount={}, maxCount={}",
                targetListId.getId(),
                currentCount,
                MAX_CARDS_PER_LIST
            );
     ...
     * 리스트 내 위치 범위를 검증합니다.
     */
    private Either<Failure, Void> validatePositionRange(
        ListId listId,
        int position
    ) {
        ...
        if (position > cardCount) {
            log.warn(
                "위치 범위 초과: listId={}, position={}, cardCount={}",
                listId.getId(),
                position,
                cardCount
            );
