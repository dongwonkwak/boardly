...
@Tag(name = "BoardList", description = "BoardList API")
public class BoardListController {
  
  private static final String TAGS = "BoardList";

  private final GetBoardListsUseCase getBoardListsUseCase;
  private final CreateBoardListUseCase createBoardListUseCase;
  private final UpdateBoardListUseCase updateBoardListUseCase;
  private final DeleteBoardListUseCase deleteBoardListUseCase;
  private final UpdateBoardListPositionUseCase updateBoardListPositionUseCase;

  @Operation(
    summary = "보드 리스트 목록 조회",
    description = "특정 보드에 속한 모든 리스트를 position 순서대로 조회합니다.",
    tags = {TAGS},
    security = @SecurityRequirement(name = "oauth2", scopes = {"read", "openid"}))
  @ApiResponses(value = {
    @ApiResponse(responseCode = "200", description = "보드 리스트 목록 조회 성공",
      content = @Content(mediaType = APPLICATION_JSON_VALUE, 
        array = @ArraySchema(schema = @Schema(implementation = BoardListResponse.class)))),
    @ApiResponse(responseCode = "403", description = "보드 접근 권한 없음",
  ...
  public ResponseEntity<?> getBoardLists(
    @Parameter(description = "조회할 보드 ID", required = true)
    @PathVariable String boardId,
    HttpServletRequest httpRequest,
    @Parameter(hidden = true) @AuthenticationPrincipal Jwt jwt) {
    
    String userId = jwt.getSubject();
    log.info("보드 리스트 목록 조회 요청: userId={}, boardId={}", userId, boardId);
    ...
    return result.fold(
      ...
      boardLists -> {
        log.info("보드 리스트 목록 조회 성공: userId={}, boardId={}, listCount={}", 
  ...
  @Operation(
    summary = "보드 리스트 생성",
    description = "특정 보드에 새로운 리스트를 생성합니다.",
    tags = {TAGS},
    security = @SecurityRequirement(name = "oauth2", scopes = {"read", "write", "openid"}))
  @ApiResponses(value = {
    @ApiResponse(responseCode = "201", description = "보드 리스트 생성 성공",
      content = @Content(mediaType = APPLICATION_JSON_VALUE, schema = @Schema(implementation = BoardListResponse.class))),
    @ApiResponse(responseCode = "403", description = "보드 접근 권한 없음",
      content = @Content(mediaType = APPLICATION_JSON_VALUE, schema = @Schema(implementation = ErrorResponse.class))),
    @ApiResponse(responseCode = "404", description = "보드를 찾을 수 없음",
      content = @Content(mediaType = APPLICATION_JSON_VALUE, schema = @Schema(implementation = ErrorResponse.class))),
  ...
  public ResponseEntity<?> createBoardList(
    @Parameter(description = "리스트를 생성할 보드 ID", required = true)
    @PathVariable String boardId,
    @Parameter(description = "보드 리스트 생성 정보", required = true)
    @RequestBody CreateBoardListRequest request,
    HttpServletRequest httpRequest,
    @Parameter(hidden = true) @AuthenticationPrincipal Jwt jwt) {
    
    String userId = jwt.getSubject();
    log.info("보드 리스트 생성 요청: userId={}, boardId={}, title={}", userId, boardId, request.getTitle());
    ...
    return result.fold(
      ApiFailureHandler::handleFailure,
      boardList -> {
        log.info("보드 리스트 생성 성공: userId={}, boardId={}, listId={}", 
  ...
  @Operation(
    summary = "보드 리스트 수정",
    description = "특정 보드 리스트의 정보를 수정합니다.",
    tags = {TAGS},
    security = @SecurityRequirement(name = "oauth2", scopes = {"read", "write", "openid"}))
  @ApiResponses(value = {
    @ApiResponse(responseCode = "200", description = "보드 리스트 수정 성공",
      content = @Content(mediaType = APPLICATION_JSON_VALUE, schema = @Schema(implementation = BoardListResponse.class))),
    @ApiResponse(responseCode = "403", description = "보드 접근 권한 없음",
      content = @Content(mediaType = APPLICATION_JSON_VALUE, schema = @Schema(implementation = ErrorResponse.class))),
    @ApiResponse(responseCode = "404", description = "보드 또는 리스트를 찾을 수 없음",
      content = @Content(mediaType = APPLICATION_JSON_VALUE, schema = @Schema(implementation = ErrorResponse.class))),
    @ApiResponse(responseCode = "422", description = "입력 값이 유효하지 않음",
      content = @Content(mediaType = APPLICATION_JSON_VALUE, schema = @Schema(implementation = ErrorResponse.class))),
    @ApiResponse(responseCode = "500", description = "서버 오류",
  ...
  public ResponseEntity<?> updateBoardList(
    @Parameter(description = "수정할 리스트 ID", required = true)
    @PathVariable String listId,
    @Parameter(description = "보드 리스트 수정 정보", required = true)
    @RequestBody UpdateBoardListRequest request,
    HttpServletRequest httpRequest,
    @Parameter(hidden = true) @AuthenticationPrincipal Jwt jwt) {
    
    String userId = jwt.getSubject();
    log.info("보드 리스트 수정 요청: userId={}, listId={}, title={}", 
    ...
    return result.fold(
      ApiFailureHandler::handleFailure,
      boardList -> {
        log.info("보드 리스트 수정 성공: userId={}, listId={}", 
  ...
  public ResponseEntity<?> deleteBoardList(
    @Parameter(description = "삭제할 리스트 ID", required = true)
    @PathVariable String listId,
    HttpServletRequest httpRequest,
    @Parameter(hidden = true) @AuthenticationPrincipal Jwt jwt) {
    
    String userId = jwt.getSubject();
    log.info("보드 리스트 삭제 요청: userId={}, listId={}", userId, listId);
    ...
    return result.fold(
      ApiFailureHandler::handleFailure,
      success -> {
        log.info("보드 리스트 삭제 성공: userId={}, listId={}", userId, listId);
  ...
  @Operation(
    summary = "보드 리스트 위치 변경",
    description = "특정 보드 리스트의 위치를 변경합니다.",
    tags = {TAGS},
    security = @SecurityRequirement(name = "oauth2", scopes = {"write", "openid"}))
  @ApiResponses(value = {
    @ApiResponse(responseCode = "200", description = "보드 리스트 위치 변경 성공",
      content = @Content(mediaType = APPLICATION_JSON_VALUE, schema = @Schema(implementation = BoardListResponse.class))),
    @ApiResponse(responseCode = "403", description = "보드 접근 권한 없음",
      content = @Content(mediaType = APPLICATION_JSON_VALUE, schema = @Schema(implementation = ErrorResponse.class))),
    @ApiResponse(responseCode = "404", description = "보드 또는 리스트를 찾을 수 없음",
  ...
  public ResponseEntity<?> updateBoardListPosition(
    @Parameter(description = "위치를 변경할 리스트 ID", required = true)
    @PathVariable String listId,
    @Parameter(description = "보드 리스트 위치 변경 정보", required = true)
    @RequestBody UpdateBoardListPositionRequest request,
    HttpServletRequest httpRequest,
    @Parameter(hidden = true) @AuthenticationPrincipal Jwt jwt) {
    
    String userId = jwt.getSubject();
    log.info("보드 리스트 위치 변경 요청: userId={}, listId={}, position={}", 
    ...
    Either<Failure, List<BoardList>> result = updateBoardListPositionUseCase.updateBoardListPosition(command);
    ...
    return result.fold(
      ...
      boardLists -> {
        ...
        log.info("보드 리스트 위치 변경 성공: userId={}, listId={}, newPosition={}, totalUpdated={}", 
                userId, updatedList.getListId().getId(), updatedList.getPosition(), boardLists.size());
