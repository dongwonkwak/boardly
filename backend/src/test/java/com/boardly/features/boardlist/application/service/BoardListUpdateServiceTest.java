...
class BoardListUpdateServiceTest {
    ...
    class UpdateBoardListTest {
        ...
        void updateBoardList_withTitleOnly_shouldReturnUpdatedBoardList() {
            // given
            UpdateBoardListCommand titleOnlyCommand =
                new UpdateBoardListCommand(
                    testListId,
                    testUserId,
                    "새로운 제목",
                    null,
            ...
            ValidationResult<UpdateBoardListCommand> validResult =
                ValidationResult.valid(titleOnlyCommand);
            when(
                boardListValidator.validateUpdateBoardList(titleOnlyCommand)
            ).thenReturn(validResult);
            when(boardListRepository.findById(testListId)).thenReturn(
                Optional.of(testBoardList)
            );
            ...
            // when
            Either<Failure, BoardList> result =
                boardListUpdateService.updateBoardList(titleOnlyCommand);

            // then
            assertThat(result.isRight()).isTrue();
            BoardList updatedList = result.get();
            assertThat(updatedList.getTitle()).isEqualTo("새로운 제목");
            assertThat(updatedList.getDescription()).isEqualTo(
                "기존 리스트 설명"
            ); // 기존 값 유지
            assertThat(updatedList.getColor()).isEqualTo(
        ...
        void updateBoardList_withColorOnly_shouldReturnUpdatedBoardList() {
            // given
            UpdateBoardListCommand colorOnlyCommand =
                new UpdateBoardListCommand(
                    testListId,
                    testUserId,
                    null,
            ...
            ValidationResult<UpdateBoardListCommand> validResult =
                ValidationResult.valid(colorOnlyCommand);
            when(
                boardListValidator.validateUpdateBoardList(colorOnlyCommand)
            ).thenReturn(validResult);
            when(boardListRepository.findById(testListId)).thenReturn(
                Optional.of(testBoardList)
            );
            ...
            // when
            Either<Failure, BoardList> result =
                boardListUpdateService.updateBoardList(colorOnlyCommand);
        ...
        void updateBoardList_withInvalidData_shouldReturnInputError() {
            ...
            List<FieldViolation> validationErrors = List.of(
                ...
                FieldViolation.builder()
                    .field("title")
                    .message("제목 길이가 너무 깁니다")
                    .rejectedValue("")
                    .build()
            );
            ValidationResult<UpdateBoardListCommand> invalidResult =
            ...
            assertThat(inputError.getMessage()).isEqualTo(
                "입력값이 유효하지 않습니다"
            );
            assertThat(inputError.getViolations()).containsExactlyElementsOf(
                validationErrors
            );
        }
        ...
        void updateBoardList_withNonExistentList_shouldReturnNotFound() {
            ...
            when(
                validationMessageResolver.getMessage(
                    "validation.boardlist.not.found"
                )
            ...
            assertThat(notFound.getMessage()).isEqualTo(
                "리스트를 찾을 수 없습니다"
            );
            assertThat(
                (Map<String, Object>) notFound.getContext()
            ).containsEntry("listId", testListId.getId());
        }
        ...
        void updateBoardList_withNonExistentBoard_shouldReturnNotFound() {
            ...
            assertThat(
                (Map<String, Object>) notFound.getContext()
            ).containsEntry("boardId", testBoardId.getId());
        }
        ...
        void updateBoardList_withUnauthorizedAccess_shouldReturnPermissionDenied() {
            ...
            UpdateBoardListCommand unauthorizedCommand =
                new UpdateBoardListCommand(
                    testListId,
                    unauthorizedUserId,
                    "새로운 제목",
            ...
            ValidationResult<UpdateBoardListCommand> validResult =
                ValidationResult.valid(unauthorizedCommand);
            when(
                boardListValidator.validateUpdateBoardList(unauthorizedCommand)
            ).thenReturn(validResult);
            when(boardListRepository.findById(testListId)).thenReturn(
                Optional.of(testBoardList)
            );
            ...
            // when
            Either<Failure, BoardList> result =
                boardListUpdateService.updateBoardList(unauthorizedCommand);

            // then
            assertThat(result.isLeft()).isTrue();
            Failure failure = result.getLeft();
            assertThat(failure).isInstanceOf(Failure.PermissionDenied.class);
            Failure.PermissionDenied permissionDenied =
                (Failure.PermissionDenied) failure;
            assertThat(permissionDenied.getErrorCode()).isEqualTo(
                "UNAUTHORIZED_ACCESS"
            );
            assertThat(permissionDenied.getMessage()).isEqualTo(
                "리스트 수정 권한이 없습니다"
            );
            assertThat(
                (Map<String, Object>) permissionDenied.getContext()
            ).containsEntry("listId", testListId.getId());
            assertThat(
                (Map<String, Object>) permissionDenied.getContext()
            ).containsEntry("userId", unauthorizedUserId.getId());
        }
        ...
        void updateBoardList_withTitleLengthExceeded_shouldReturnBusinessRuleViolation() {
            ...
            UpdateBoardListCommand longTitleCommand =
                new UpdateBoardListCommand(
                    testListId,
                    testUserId,
                    longTitle,
            ...
            ValidationResult<UpdateBoardListCommand> validResult =
                ValidationResult.valid(longTitleCommand);
            when(
                boardListValidator.validateUpdateBoardList(longTitleCommand)
            ).thenReturn(validResult);
            when(boardListRepository.findById(testListId)).thenReturn(
                Optional.of(testBoardList)
            );
            ...
            // when
            Either<Failure, BoardList> result =
                boardListUpdateService.updateBoardList(longTitleCommand);
            ...
            assertThat(businessRuleViolation.getErrorCode()).isEqualTo(
                "TITLE_LENGTH_EXCEEDED"
            );
            assertThat(businessRuleViolation.getMessage()).contains(
                "리스트 제목은 최대 100자까지 입력할 수 있습니다"
            );
            assertThat(
                (Map<String, Object>) businessRuleViolation.getContext()
            ).containsEntry("listId", testListId.getId());
            assertThat(
                (Map<String, Object>) businessRuleViolation.getContext()
        ...
        void updateBoardList_withSaveException_shouldReturnInternalError() {
            ...
            when(
                validationMessageResolver.getMessage(
                    "validation.boardlist.update.error"
                )
            ...
            Failure.InternalError internalError =
                (Failure.InternalError) failure;
            assertThat(internalError.getErrorCode()).isEqualTo(
                "BOARD_LIST_UPDATE_ERROR"
            );
            assertThat(internalError.getMessage()).isEqualTo(
                "리스트 수정 중 오류가 발생했습니다"
            );
            assertThat(
                (Map<String, Object>) internalError.getContext()
            ).containsEntry("listId", testListId.getId());
            assertThat(
                (Map<String, Object>) internalError.getContext()
            ).containsEntry("error", "저장 실패");
        }
    }
    ...
    class UpdateBoardListPositionTest {
        ...
        void updateBoardListPosition_withInvalidData_shouldReturnInputError() {
            ...
            ValidationResult<UpdateBoardListPositionCommand> invalidResult =
                ValidationResult.invalid(
                    io.vavr.collection.List.ofAll(validationErrors)
                );
            when(
        ...
        void updateBoardListPosition_withUnauthorizedAccess_shouldReturnPermissionDenied() {
            // given
            UserId unauthorizedUserId = new UserId("unauthorized-user-456");
            UpdateBoardListPositionCommand unauthorizedCommand =
                new UpdateBoardListPositionCommand(
                    testListId,
                    unauthorizedUserId,
                    2
                );
        ...
        void updateBoardListPosition_withPolicyViolation_shouldReturnBusinessRuleViolation() {
            ...
            when(
                ...
                Either.left(
                    Failure.ofBusinessRuleViolation("이동할 수 없는 위치입니다")
                )
            );
            when(
                validationMessageResolver.getMessage(
            ...
            assertThat(businessRuleViolation.getMessage()).isEqualTo(
                "리스트 이동 정책을 위반했습니다"
            );
            assertThat(
                (Map<String, Object>) businessRuleViolation.getContext()
    ...
    class BoundaryValueTest {
        ...
        void updateBoardList_withExactMaxTitleLength_shouldSucceed() {
            ...
            ValidationResult<UpdateBoardListCommand> validResult =
                ValidationResult.valid(exactMaxCommand);
            when(
                boardListValidator.validateUpdateBoardList(exactMaxCommand)
            ).thenReturn(validResult);
            when(boardListRepository.findById(testListId)).thenReturn(
                Optional.of(testBoardList)
            );
            ...
            // when
            Either<Failure, BoardList> result =
                boardListUpdateService.updateBoardList(exactMaxCommand);
        ...
        void updateBoardListPosition_toPositionZero_shouldSucceed() {
            // given
            UpdateBoardListPositionCommand zeroPositionCommand =
                new UpdateBoardListPositionCommand(testListId, testUserId, 0);
            ...
            ValidationResult<UpdateBoardListPositionCommand> validResult =
                ValidationResult.valid(zeroPositionCommand);
            when(
                boardListValidator.validateUpdateBoardListPosition(
                    zeroPositionCommand
                )
            ...
            when(
                boardListMovePolicy.canMoveWithinSameBoard(any(), eq(0))
            ).thenReturn(Either.right(null));
            when(
                boardListMovePolicy.hasPositionChanged(any(), eq(0))
            ).thenReturn(true);
            when(boardListRepository.saveAll(anyList())).thenReturn(allLists);
        ...
        void updateBoardListPosition_toLastPosition_shouldSucceed() {
            // given
            UpdateBoardListPositionCommand lastPositionCommand =
                new UpdateBoardListPositionCommand(testListId, testUserId, 2);
            ...
            ValidationResult<UpdateBoardListPositionCommand> validResult =
                ValidationResult.valid(lastPositionCommand);
            when(
                boardListValidator.validateUpdateBoardListPosition(
                    lastPositionCommand
                )
        ...
        void updateBoardList_withEmptyTitle_shouldSucceed() {
            // given
            UpdateBoardListCommand emptyTitleCommand =
                new UpdateBoardListCommand(
                    testListId,
                    testUserId,
                    "",
            ...
            ValidationResult<UpdateBoardListCommand> validResult =
                ValidationResult.valid(emptyTitleCommand);
            when(
                boardListValidator.validateUpdateBoardList(emptyTitleCommand)
            ).thenReturn(validResult);
            when(boardListRepository.findById(testListId)).thenReturn(
                Optional.of(testBoardList)
            );
            ...
            // when
            Either<Failure, BoardList> result =
                boardListUpdateService.updateBoardList(emptyTitleCommand);
    ...
    class IntegrationTest {
        ...
        void updateBoardListThenPosition_shouldWorkCorrectly() {
            // given - 리스트 수정
            ValidationResult<UpdateBoardListCommand> validUpdateResult =
                ValidationResult.valid(validUpdateCommand);
            when(
                boardListValidator.validateUpdateBoardList(validUpdateCommand)
            ...
            // when - 리스트 수정
            Either<Failure, BoardList> updateResult =
                boardListUpdateService.updateBoardList(validUpdateCommand);
            ...
            ValidationResult<
                UpdateBoardListPositionCommand
            > validPositionResult = ValidationResult.valid(
                validPositionCommand
            );
            when(
                boardListValidator.validateUpdateBoardListPosition(
            ...
            // when - 위치 변경
            Either<Failure, List<BoardList>> positionResult =
                boardListUpdateService.updateBoardListPosition(
                    validPositionCommand
                );
