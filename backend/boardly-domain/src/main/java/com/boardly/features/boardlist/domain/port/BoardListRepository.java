package com.boardly.features.boardlist.domain.port;
...
public interface BoardListRepository {
     ...
     */
    List<BoardList> findByBoardIdAndPositionGreaterThan(
        BoardId boardId,
        int position
    );
     ...
     * @return 범위 내의 리스트 목록
     */
    List<BoardList> findByBoardIdAndPositionBetween(
        BoardId boardId,
        int startPosition,
        int endPosition
    );
     ...
     */
    List<BoardList> findByBoardIdAndTitleContaining(
        BoardId boardId,
        String title
    );
