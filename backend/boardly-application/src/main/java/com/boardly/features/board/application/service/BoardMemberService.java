...
import com.boardly.features.board.application.command.RemoveBoardMemberCommand;
...
public class BoardMemberService
    ...
    private UserNameDto getUserName(UserId userId) {
        return userFinder
            ...
            .orElse(new UserNameDto(userId.getId(), "기본이름"));
    }
    ...
    private String getBoardName(BoardId boardId) {
        return boardRepository.findBoardNameById(boardId).orElse("기본보드");
    }

    // ==================== ADD BOARD MEMBER ====================

    @Override
    public Either<Failure, BoardMember> addBoardMember(
        AddBoardMemberCommand command
    ) {
        log.info(
            "보드 멤버 추가 시작: boardId={}, userId={}, role={}, requestedBy={}",
        ...
        return validateRequestedUser(command.requestedBy())
            ...
            .flatMap(boardMember ->
                saveBoardMemberWithActivity(boardMember, command)
            );
    }

    private Either<Failure, AddBoardMemberCommand> validateAddMemberInput(
        AddBoardMemberCommand command
    ) {
        var validationResult = boardValidator.validateAddMember(command);
        if (validationResult.isInvalid()) {
            log.warn(
                "보드 멤버 추가 검증 실패: boardId={}, violations={}",
                command.boardId() != null ? command.boardId().getId() : "null",
    ...
    private Either<Failure, Board> findExistingBoard(
        AddBoardMemberCommand command
    ) {
    ...
    private Either<Failure, Board> verifyManagementPermission(
        AddBoardMemberCommand command,
        Board board
    ) {
        return boardPermissionService
            .canManageBoardMembers(board.getBoardId(), command.requestedBy())
            .flatMap(canManage -> {
                if (!canManage) {
                    log.warn(
                        "보드 멤버 관리 권한 없음: boardId={}, requestedBy={}",
    ...
    private Either<Failure, AddBoardMemberCommand> checkMemberAlreadyExists(
        AddBoardMemberCommand command,
        Board board
    ) {
        ...
        if (exists) {
            log.warn(
                "이미 보드 멤버로 등록된 사용자: boardId={}, userId={}",
                board.getBoardId().getId(),
            ...
            return Either.left(
                Failure.ofConflict(
                    messageResolver.getMessage(
                        "validation.board.member.already.exists"
                    )
                )
            );
        }
    ...
    private Either<Failure, BoardMember> createBoardMember(
        AddBoardMemberCommand command
    ) {
        try {
            ...
            log.debug(
                "보드 멤버 생성 완료: boardId={}, userId={}, role={}",
                command.boardId().getId(),
                command.userId().getId(),
                command.role()
            );
            ...
            log.error(
                "보드 멤버 생성 중 오류 발생: boardId={}, error={}",
                command.boardId().getId(),
                e.getMessage(),
                e
            );
    ...
    private Either<Failure, BoardMember> saveBoardMemberWithActivity(
        BoardMember boardMember,
        AddBoardMemberCommand command
    ) {
        return saveBoardMember(boardMember).peek(savedMember ->
            logBoardMemberAddedActivity(savedMember, command)
        );
    }

    private Either<Failure, BoardMember> saveBoardMember(
        BoardMember boardMember
    ) {
        return boardMemberRepository
            ...
            .peek(savedMember -> {
                log.info(
                    "보드 멤버 추가 완료: memberId={}, boardId={}, userId={}, role={}",
    ...
    private void logBoardMemberAddedActivity(
    ...
    private Either<Failure, Void> validateRequestedUser(UserId requestedBy) {
        if (requestedBy != null && !userFinder.userExists(requestedBy)) {
            return Either.left(
                Failure.ofNotFound(
    ...
    @Override
    public Either<Failure, Void> removeBoardMember(
        RemoveBoardMemberCommand command
    ) {
        log.info(
            "보드 멤버 제거 시작: boardId={}, targetUserId={}, requestedBy={}",
            command.boardId() != null ? command.boardId().getId() : "null",
            command.targetUserId() != null
                ? command.targetUserId().getId()
                : "null",
            command.requestedBy() != null
                ? command.requestedBy().getId()
                : "null"
        ...
        return validateRequestedUser(command.requestedBy())
            ...
            .flatMap(member ->
                performMemberRemovalWithActivity(member, command)
            );
    }

    private Either<Failure, RemoveBoardMemberCommand> validateRemoveMemberInput(
        RemoveBoardMemberCommand command
    ) {
        var validationResult = boardValidator.validateRemoveMember(command);
        if (validationResult.isInvalid()) {
            log.warn(
                "보드 멤버 제거 검증 실패: boardId={}, violations={}",
                command.boardId() != null ? command.boardId().getId() : "null",
                validationResult.getErrorsAsCollection()
            );
            return Either.left(
    ...
    private Either<Failure, Board> findBoardForMemberRemoval(
        RemoveBoardMemberCommand command
    ) {
        Optional<Board> boardOpt = boardRepository.findById(command.boardId());
        if (boardOpt.isEmpty()) {
    ...
    private Either<Failure, Board> verifyRemovePermission(
        RemoveBoardMemberCommand command,
        Board board
    ) {
        return boardPermissionService
    ...
    private Either<Failure, BoardMember> findExistingMember(
        RemoveBoardMemberCommand command,
        Board board
    ) {
        Optional<BoardMember> memberOpt =
        ...
        if (memberOpt.isEmpty()) {
            log.warn(
                "보드 멤버를 찾을 수 없음: boardId={}, targetUserId={}",
                board.getBoardId().getId(),
                command.targetUserId().getId()
            );
            return Either.left(
                Failure.ofNotFound(
                    messageResolver.getMessage(
                        "validation.board.member.not.found"
                    )
                )
            );
        }
        ...
        if (board.getOwnerId().equals(command.targetUserId())) {
            log.warn(
                "보드 소유자 제거 시도: boardId={}, targetUserId={}",
                board.getBoardId().getId(),
                command.targetUserId().getId()
            );
            return Either.left(
                Failure.ofConflict(
                    messageResolver.getMessage(
                        "validation.board.owner.removal.denied"
                    )
                )
            );
        }
    ...
    private Either<Failure, Void> performMemberRemovalWithActivity(
        BoardMember member,
        RemoveBoardMemberCommand command
    ) {
        return logBoardMemberRemovedActivity(member, command).flatMap(ignored ->
            deleteBoardMember(member)
        );
    }

    private Either<Failure, Void> logBoardMemberRemovedActivity(
        BoardMember member,
        RemoveBoardMemberCommand command
    ) {
    ...
    private Either<Failure, Void> deleteBoardMember(BoardMember member) {
        return boardMemberRepository
            ...
            .peek(deletedMember -> {
                log.info(
                    "보드 멤버 제거 완료: memberId={}, boardId={}, userId={}",
    ...
    @Override
    public Either<Failure, BoardMember> updateBoardMemberRole(
        UpdateBoardMemberRoleCommand command
    ) {
        log.info(
            "보드 멤버 역할 변경 시작: boardId={}, targetUserId={}, newRole={}, requestedBy={}",
            command.boardId() != null ? command.boardId().getId() : "null",
            command.targetUserId() != null
                ? command.targetUserId().getId()
                : "null",
    ...
    private Either<
        Failure,
        UpdateBoardMemberRoleCommand
    > validateUpdateRoleInput(UpdateBoardMemberRoleCommand command) {
        var validationResult = boardValidator.validateUpdateMemberRole(command);
        if (validationResult.isInvalid()) {
            log.warn(
                "보드 멤버 역할 변경 검증 실패: boardId={}, violations={}",
                command.boardId() != null ? command.boardId().getId() : "null",
                validationResult.getErrorsAsCollection()
            );
            return Either.left(
    ...
    private Either<Failure, Board> findBoardForRoleUpdate(
        UpdateBoardMemberRoleCommand command
    ) {
        Optional<Board> boardOpt = boardRepository.findById(command.boardId());
        if (boardOpt.isEmpty()) {
        ...
        if (board.isArchived()) {
            log.warn(
                "아카이브된 보드에서 멤버 역할 변경 시도: boardId={}",
                command.boardId().getId()
            );
            return Either.left(
                Failure.ofConflict(
    ...
    private Either<Failure, Board> verifyRoleUpdatePermission(
        UpdateBoardMemberRoleCommand command,
        Board board
    ) {
        return boardPermissionService
    ...
    private Either<Failure, BoardMember> findMemberForRoleUpdate(
        UpdateBoardMemberRoleCommand command,
        Board board
    ) {
        return findMemberByBoardAndUser(command, board)
            .flatMap(member ->
                validateRoleChangeAllowed(command, board, member)
            )
            .flatMap(member -> checkRoleChangeNeeded(command, member));
    }

    private Either<Failure, BoardMember> findMemberByBoardAndUser(
        UpdateBoardMemberRoleCommand command,
        Board board
    ) {
        Optional<BoardMember> memberOpt =
    ...
    private Either<Failure, BoardMember> validateRoleChangeAllowed(
        UpdateBoardMemberRoleCommand command,
        Board board,
        BoardMember member
    ) {
        ...
        if (board.getOwnerId().equals(command.targetUserId())) {
            log.warn(
                "보드 소유자 역할 변경 시도: boardId={}, targetUserId={}",
                board.getBoardId().getId(),
                command.targetUserId().getId()
            );
            return Either.left(
                Failure.ofConflict(
                    messageResolver.getMessage(
                        "validation.board.owner.role.change.denied"
                    )
                )
            );
        }
    ...
    private Either<Failure, BoardMember> checkRoleChangeNeeded(
        UpdateBoardMemberRoleCommand command,
        BoardMember member
    ) {
        ...
        if (member.getRole().equals(command.newRole())) {
            log.info(
                "멤버가 이미 해당 역할을 가지고 있음: boardId={}, targetUserId={}, role={}",
                member.getBoardId().getId(),
                command.targetUserId().getId(),
                command.newRole()
            );
    ...
    private Either<Failure, BoardMember> performRoleUpdate(
        BoardMember member,
        UpdateBoardMemberRoleCommand command
    ) {
        return changeMemberRole(member, command).flatMap(updatedMember ->
            saveUpdatedMember(updatedMember, command)
        );
    }

    private Either<Failure, BoardMember> changeMemberRole(
        BoardMember member,
        UpdateBoardMemberRoleCommand command
    ) {
        try {
            ...
            log.debug(
                "보드 멤버 역할 변경 처리 완료: memberId={}, oldRole={}, newRole={}",
                member.getMemberId().getId(),
                oldRole,
                command.newRole()
            );
            ...
            log.error(
                "보드 멤버 역할 변경 중 오류 발생: memberId={}, error={}",
                member.getMemberId().getId(),
                e.getMessage(),
                e
            );
            return Either.left(
                Failure.ofInternalError(
                    e.getMessage(),
                    "BOARD_MEMBER_ROLE_UPDATE_ERROR",
                    null
                )
            );
        }
    ...
    private Either<Failure, BoardMember> saveUpdatedMember(
        BoardMember member,
        UpdateBoardMemberRoleCommand command
    ) {
        return boardMemberRepository
            ...
            .peek(savedMember -> {
                log.info(
                    "보드 멤버 역할 변경 완료: memberId={}, boardId={}, userId={}, oldRole={}, newRole={}",
                    savedMember.getMemberId().getId(),
                    savedMember.getBoardId().getId(),
                    savedMember.getUserId().getId(),
                ...
                activityHelper.logBoardActivity(
                    ...
                    Map.of(
                        ...
                        userName.lastName(),
                        "oldRole",
                        member.getRole().toString(), // 변경 전 역할
                        "newRole",
