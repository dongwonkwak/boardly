...
public class CardPolicyConfigImpl implements CardPolicyConfig {
    ...
    public int getMaxCardsPerList() {
        ...
        if (configValue <= 0) {
            log.debug(
                "리스트당 최대 카드 개수 설정값이 유효하지 않음: {}. 기본값 {} 사용",
                configValue,
                Defaults.MAX_CARDS_PER_LIST
            );
    ...
    public int getMaxTitleLength() {
        ...
        if (configValue <= 0) {
            log.debug(
                "카드 제목 최대 길이 설정값이 유효하지 않음: {}. 기본값 {} 사용",
                configValue,
    ...
    public int getMaxDescriptionLength() {
        ...
        if (configValue <= 0) {
            log.debug(
                "카드 설명 최대 길이 설정값이 유효하지 않음: {}. 기본값 {} 사용",
                configValue,
    ...
    public int getMaxSearchResults() {
        ...
        if (configValue <= 0) {
            log.debug(
                "검색 결과 최대 개수 설정값이 유효하지 않음: {}. 기본값 {} 사용",
                configValue,
