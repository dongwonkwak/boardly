...
import com.boardly.features.user.domain.port.UserRepository;
...
public class UserRepositoryImpl implements UserRepository {
    ...
    public Either<Failure, User> save(User user) {
        try {
            ...
            if (user.isNew()) {
                ...
                // 기존 객체 업데이트
                log.debug(
                    "기존 사용자 업데이트: userId={}, email={}",
                    user.getUserId().getId(),
                    user.getEmail()
                );
                Optional<UserEntity> existingEntity =
                    userJpaRepository.findById(user.getUserId().getId());
    ...
    @Override
    public Optional<String> findUserNameById(UserId userId) {
        log.debug("findUserNameById: userId={}", userId.getId());
        return userJpaRepository
            .findUserNameById(userId.getId())
            .map(entity ->
                String.format(
                    "%s %s",
                    entity.getFirstName(),
