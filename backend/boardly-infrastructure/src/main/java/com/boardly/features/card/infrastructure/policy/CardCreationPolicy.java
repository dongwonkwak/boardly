...
public class CardCreationPolicy {
    ...
    public Either<Failure, Void> canCreateCard(ListId listId) {
        ...
        return checkCardCountLimit(listId).peek(v ->
            log.debug("카드 생성 정책 검증 성공: listId={}", listId.getId())
        );
    }
    ...
    private Either<Failure, Void> checkCardCountLimit(ListId listId) {
        ...
        log.debug(
            "현재 카드 개수 확인: listId={}, currentCount={}, maxCount={}",
            listId.getId(),
            currentCount,
            maxCards
        );
        ...
        if (currentCount >= maxCards) {
            ...
            return Either.left(
                Failure.ofForbidden(
                    String.format(
                        "리스트당 최대 %d개의 카드만 생성할 수 있습니다. (현재: %d개)",
                        maxCards,
                        currentCount
                    )
                )
