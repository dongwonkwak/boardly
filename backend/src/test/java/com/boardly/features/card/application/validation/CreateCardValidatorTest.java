...
class CreateCardValidatorTest {
  ...
  void setUp() {
    ...
    lenient().when(messageSource.getMessage(anyString(), any(), any(Locale.class)))
        .thenAnswer(invocation -> {
          ...
          return switch (key) {
            ...
            case "validation.listId.required" -> "List ID is required";
    ...
    createCardValidator = new CreateCardValidator(commonValidationRules);
  }
  ...
  @Test
  @DisplayName("유효한 카드 생성 정보는 검증을 통과해야 한다")
  void validate_withValidData_shouldBeValid() {
    // given
    CreateCardCommand command = createValidCommand();

    // when
    ValidationResult<CreateCardCommand> result = createCardValidator.validate(command);
  ...
  @ParameterizedTest
  @DisplayName("유효한 제목으로 카드 생성 시 검증을 통과해야 한다")
  @MethodSource("validTitleTestData")
  void validate_withValidTitle_shouldBeValid(String title) {
    // given
    CreateCardCommand command = new CreateCardCommand(title, "설명", new ListId(), new UserId());
  ...
  @ParameterizedTest
  @DisplayName("유효하지 않은 제목으로 카드 생성 시 검증에 실패해야 한다")
  ...
  @ParameterizedTest
  @DisplayName("너무 긴 제목으로 카드 생성 시 검증에 실패해야 한다")
  ...
  @ParameterizedTest
  @DisplayName("HTML 태그가 포함된 제목으로 카드 생성 시 검증에 실패해야 한다")
  ...
  @ParameterizedTest
  @DisplayName("유효한 설명으로 카드 생성 시 검증을 통과해야 한다")
  @MethodSource("validDescriptionTestData")
  void validate_withValidDescription_shouldBeValid(String description) {
    // given
    CreateCardCommand command = new CreateCardCommand("유효한 카드 제목", description, new ListId(), new UserId());
  ...
  @ParameterizedTest
  @DisplayName("HTML 태그가 포함된 설명으로 카드 생성 시 검증에 실패해야 한다")
  ...
  @ParameterizedTest
  @DisplayName("너무 긴 설명으로 카드 생성 시 검증에 실패해야 한다")
  ...
  @Test
  @DisplayName("listId가 null이면 검증에 실패해야 한다")
  void validate_withNullListId_shouldBeInvalid() {
    // given
    CreateCardCommand command = new CreateCardCommand(
        "유효한 카드 제목",
        "설명",
    ...
    assertThat(result.getErrors().get(0).message()).isEqualTo("List ID is required");
  }
  ...
  void validate_withMultipleInvalidFields_shouldReturnAllErrors() {
    ...
    assertThat(result.getErrors()).extracting("field")
        .containsExactlyInAnyOrder("title", "description", "listId", "userId");
  }
