...
public class BoardListRepositoryImpl implements BoardListRepository {
    ...
    @Override
    public List<BoardList> findByBoardIdAndPositionGreaterThan(
        BoardId boardId,
        int position
    ) {
        log.debug(
            "보드 ID와 위치로 리스트 조회 시작: boardId={}, position={}",
            boardId.getId(),
            position
        );
        var entities =
            boardListJpaRepository.findByBoardIdAndPositionGreaterThan(
                boardId.getId(),
                position
            );
        List<BoardList> boardLists = entities
            .stream()
            .map(boardListMapper::toDomain)
            .toList();
        log.debug(
            "보드 ID와 위치로 리스트 조회 완료: boardId={}, 리스트 개수={}",
            boardId.getId(),
            boardLists.size()
        );
        return boardLists;
    ...
    @Override
    public List<BoardList> findByBoardIdAndPositionBetween(
        BoardId boardId,
        int startPosition,
        int endPosition
    ) {
        log.debug(
            "보드 ID와 위치 범위로 리스트 조회 시작: boardId={}, startPosition={}, endPosition={}",
            boardId.getId(),
            startPosition,
            endPosition
        );
        var entities = boardListJpaRepository.findByBoardIdAndPositionBetween(
            boardId.getId(),
            startPosition,
            endPosition
        );
        var boardLists = entities
            .stream()
            .map(boardListMapper::toDomain)
            .toList();
        log.debug(
            "보드 ID와 위치 범위로 리스트 조회 완료: boardId={}, 리스트 개수={}",
            boardId.getId(),
            boardLists.size()
        );
        return boardLists;
    ...
    public Optional<BoardList> findById(ListId listId) {
        ...
        var boardList = boardListJpaRepository
            .findById(listId.getId())
            .map(boardListMapper::toDomain);

        if (boardList.isPresent()) {
            log.debug(
                "리스트 조회 완료: listId={}, title={}",
                listId.getId(),
    ...
    public List<BoardList> findByBoardIdOrderByPosition(BoardId boardId) {
        ...
        var entities = boardListJpaRepository.findByBoardIdOrderByPosition(
            boardId.getId()
        );
        var boardLists = entities
            .stream()
            ...
            .toList();
        log.debug(
            "보드 ID로 리스트 조회 완료: boardId={}, 리스트 개수={}",
            boardId.getId(),
            boardLists.size()
        );
        return boardLists;
    ...
    @Override
    public Optional<BoardList> findByIdAndBoardId(
        ListId listId,
        BoardId boardId
    ) {
        log.debug(
            "리스트 ID와 보드 ID로 리스트 조회 시작: listId={}, boardId={}",
            listId.getId(),
            boardId.getId()
        );
        var entity = boardListJpaRepository.findByListIdAndBoardId(
            listId.getId(),
            boardId.getId()
        );
        ...
        if (boardList.isPresent()) {
            log.debug(
                "리스트 ID와 보드 ID로 리스트 조회 완료: listId={}, boardId={}, title={}",
                listId.getId(),
                boardId.getId(),
                boardList.get().getTitle()
            );
    ...
    @Override
    public List<BoardList> findByBoardIdAndTitleContaining(
        BoardId boardId,
        String title
    ) {
        log.debug(
            "보드 ID와 제목으로 리스트 조회 시작: boardId={}, title={}",
            boardId.getId(),
            title
        );
        var entities = boardListJpaRepository.findByBoardIdAndTitleContaining(
            boardId.getId(),
            title
        );
        var boardLists = entities
            ...
            .toList();
        log.debug(
            "보드 ID와 제목으로 리스트 조회 완료: boardId={}, 리스트 개수={}",
            boardId.getId(),
            boardLists.size()
        );
        return boardLists;
    ...
    public Optional<Integer> findMaxPositionByBoardId(BoardId boardId) {
        log.debug(
            "보드 ID로 가장 높은 위치 조회 시작: boardId={}",
            boardId.getId()
        );
        var maxPosition = boardListJpaRepository.findMaxPositionByBoardId(
            boardId.getId()
        );
        log.debug(
            "보드 ID로 가장 높은 위치 조회 완료: boardId={}, 위치={}",
            boardId.getId(),
            maxPosition.orElse(0)
        );
        return maxPosition;
    }
