...
class CardMemberValidatorTest {
    ...
    @BeforeEach
    void setUp() {
        cardMemberValidator = new CardMemberValidator(
    ...
    class AssignCardMemberValidationTest {
        ...
        void validateAssign_WithValidCommand_ShouldReturnSuccess() {
            ...
            // when
            ValidationResult<AssignCardMemberCommand> result =
                cardMemberValidator.validateAssign(command);
        ...
        void validateAssign_WithSameMemberAndRequester_ShouldReturnFailure() {
            ...
            assertThat(result.getErrors()).hasSize(1);
            FieldViolation violation = result.getErrors().get(0);
            assertThat(violation.getField()).isEqualTo("memberId");
            assertThat(violation.getMessage()).isEqualTo(
                "멤버와 요청자가 동일한 사용자일 수 없습니다"
            );
        }
    }
    ...
    class UnassignCardMemberValidationTest {
        ...
        void validateUnassign_WithValidCommand_ShouldReturnSuccess() {
            ...
            // when
            ValidationResult<UnassignCardMemberCommand> result =
                cardMemberValidator.validateUnassign(command);
    ...
    class IntegrationValidationTest {
        ...
        void allValidationMethods_ShouldWorkCorrectly() {
            ...
            AssignCardMemberCommand assignCommand = new AssignCardMemberCommand(
                cardId,
                memberId,
                requesterId
            );
            UnassignCardMemberCommand unassignCommand =
                new UnassignCardMemberCommand(cardId, memberId, requesterId);

            setupAllValidatorsToSuccess();

            // when
            ValidationResult<AssignCardMemberCommand> assignResult =
                cardMemberValidator.validateAssign(assignCommand);
            ValidationResult<UnassignCardMemberCommand> unassignResult =
                cardMemberValidator.validateUnassign(unassignCommand);
        ...
        void validationWithDifferentUserIds_ShouldSucceed() {
            ...
            AssignCardMemberCommand assignCommand1 =
                new AssignCardMemberCommand(cardId, memberId1, requesterId);
            AssignCardMemberCommand assignCommand2 =
                new AssignCardMemberCommand(cardId, memberId2, requesterId);

            setupAllValidatorsToSuccess();

            // when
            ValidationResult<AssignCardMemberCommand> result1 =
                cardMemberValidator.validateAssign(assignCommand1);
            ValidationResult<AssignCardMemberCommand> result2 =
                cardMemberValidator.validateAssign(assignCommand2);
