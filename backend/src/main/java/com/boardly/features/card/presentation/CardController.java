...
public class CardController {
    ...
    public ResponseEntity<?> createCard(
        ...
        return result.fold(
                ...
                card -> {
                    log.info("카드 생성 성공: cardId={}, title={}", card.getCardId().getId(), card.getTitle());
    ...
    public ResponseEntity<?> getCard(
        ...
        return result.fold(
                failureHandler::handleFailure,
                card -> {
                    log.info("카드 조회 성공: cardId={}, title={}", card.getCardId().getId(), card.getTitle());
    ...
    public ResponseEntity<?> updateCard(
        ...
        return result.fold(
                failureHandler::handleFailure,
                card -> {
                    log.info("카드 수정 성공: cardId={}, title={}", card.getCardId().getId(), card.getTitle());
                    return ResponseEntity.ok(CardResponse.from(card));
                });
    }
        ...
        return result.fold(
                ...
                card -> {
                    log.info("카드 이동 성공: cardId={}, newListId={}, newPosition={}",
                            card.getCardId().getId(), card.getListId().getId(), card.getPosition());
                    return ResponseEntity.ok(CardResponse.from(card));
                });
    }
        ...
        return result.fold(
                ...
                card -> {
                    log.info("카드 복제 성공: originalCardId={}, clonedCardId={}, title={}, listId={}",
                            cardId, card.getCardId().getId(), card.getTitle(), card.getListId().getId());
                    return ResponseEntity.status(HttpStatus.CREATED).body(CardResponse.from(card));
                });
    }
    ...
    public ResponseEntity<?> searchCards(
        ...
        return result.fold(
                failureHandler::handleFailure,
                cards -> {
                    log.info("카드 검색 성공: listId={}, searchTerm={}, 검색 결과 개수={}", listId, searchTerm, cards.size());
                    List<CardResponse> responses = cards.stream()
                            .map(CardResponse::from)
                            .toList();
                    return ResponseEntity.ok(responses);
