...
class ArchiveBoardServiceTest {
  ...
  void archiveBoard_withInvalidData_shouldReturnValidationFailure() {
    ...
    assertThat(inputError.getMessage()).contains("입력 데이터가 올바르지 않습니다");
  ...
  @Test
  @DisplayName("권한이 없는 사용자가 보드를 아카이브하려는 경우 Forbidden 오류를 반환해야 한다")
  void archiveBoard_withUnauthorizedUser_shouldReturnForbiddenFailure() {
    // given
    ArchiveBoardCommand command = new ArchiveBoardCommand(boardId, otherUserId);
    Board activeBoard = createActiveBoard(ownerId);

    when(messageResolver.getMessage("validation.board.archive.access.denied"))
  ...
  void archiveBoard_withArchiveException_shouldReturnInternalServerError() {
    ...
    assertThat(result.getLeft().getMessage()).isEqualTo("아카이브 처리 중 오류");
  ...
  @Test
  @DisplayName("언아카이브 시 권한이 없는 사용자가 보드를 언아카이브하려는 경우 Forbidden 오류를 반환해야 한다")
  void unarchiveBoard_withUnauthorizedUser_shouldReturnForbiddenFailure() {
    // given
    ArchiveBoardCommand command = new ArchiveBoardCommand(boardId, otherUserId);
    Board archivedBoard = createArchivedBoard(ownerId);
  ...
  @Test
  @DisplayName("언아카이브 시 보드를 찾을 수 없는 경우 NotFound 오류를 반환해야 한다")
  void unarchiveBoard_withNonExistentBoard_shouldReturnNotFoundFailure() {
    // given
    ArchiveBoardCommand command = createValidCommand();
