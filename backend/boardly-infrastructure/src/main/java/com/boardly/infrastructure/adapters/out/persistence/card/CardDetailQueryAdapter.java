...
public class CardDetailQueryAdapter implements CardDetailQuery {
    ...
    @Override
    public Optional<CardDetail> findCardDetailById(
        CardId cardId,
        UserId userId
    ) {
        log.info(
            "카드 상세 정보 조회: cardId={}, userId={}",
            cardId.getId(),
            userId.getId()
        );
    ...
    private Optional<CardDetailData> enrichWithMemberInfo(CardDetailData data) {
        List<CardMember> assignees = findAssignees(
            data.card().getCardId()
        ).orElse(List.of());
     ...
     * 첨부파일 정보로 카드 정보 보강
     */
    private Optional<CardDetailData> enrichWithAttachmentInfo(
        CardDetailData data
    ) {
        List<Object> attachments = findAttachments(
            data.card().getCardId()
        ).orElse(List.of());
     ...
     * 댓글 정보로 카드 정보 보강
     */
    private Optional<CardDetailData> enrichWithCommentInfo(
        CardDetailData data
    ) {
        List<Object> comments = findComments(data.card().getCardId()).orElse(
            List.of()
        );
     ...
     * 보드 멤버 정보로 카드 정보 보강
     */
    private Optional<CardDetailData> enrichWithBoardMemberInfo(
        CardDetailData data
    ) {
        List<Object> boardMembers = findBoardMembers(
            data.card().getListId().getId()
        ).orElse(List.of());
     ...
     * 보드 라벨 정보로 카드 정보 보강
     */
    private Optional<CardDetailData> enrichWithBoardLabelInfo(
        CardDetailData data
    ) {
        List<Label> boardLabels = findBoardLabels(
            data.card().getListId().getId()
        ).orElse(List.of());
     ...
     * 활동 내역 정보로 카드 정보 보강
     */
    private Optional<CardDetailData> enrichWithActivityInfo(
        CardDetailData data
    ) {
        List<Object> activities = findActivities(
            data.card().getCardId()
        ).orElse(List.of());
     ...
     * 생성자 정보로 카드 정보 보강
     */
    private Optional<CardDetailData> enrichWithCreatorInfo(
        CardDetailData data
    ) {
    ...
    private CardDetail buildCardDetail(CardDetailData data) {
        log.debug(
            "CardDetail 객체 생성: cardId={}",
            data.card().getCardId().getId()
        );
