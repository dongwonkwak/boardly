...
public class CardClonePolicy {
    ...
    public Either<Failure, Void> canCloneWithinSameList(Card originalCard) {
        log.debug(
            "같은 리스트 내 카드 복제 정책 검증: cardId={}, listId={}",
            originalCard.getCardId().getId(),
            originalCard.getListId().getId()
        );

        return checkCardCountLimit(originalCard.getListId()).peek(v ->
            log.debug("같은 리스트 내 카드 복제 정책 검증 성공")
        );
    }
     ...
     * @return 성공 시 Right(Void), 실패 시 Left(Failure)
     */
    public Either<Failure, Void> canCloneToAnotherList(
        Card originalCard,
        ListId targetListId
    ) {
        log.debug(
            "다른 리스트로 카드 복제 정책 검증: cardId={}, sourceListId={}, targetListId={}",
            originalCard.getCardId().getId(),
            originalCard.getListId().getId(),
            targetListId.getId()
        );

        return checkCardCountLimit(targetListId).peek(v ->
            log.debug("다른 리스트로 카드 복제 정책 검증 성공")
        );
    }
    ...
    private Either<Failure, Void> checkCardCountLimit(ListId listId) {
        ...
        log.debug(
            "카드 개수 제한 확인: listId={}, currentCount={}, maxCount={}",
            listId.getId(),
            currentCount,
            MAX_CARDS_PER_LIST
        );
        ...
        if (currentCount >= MAX_CARDS_PER_LIST) {
            log.warn(
                "카드 개수 제한 초과: listId={}, currentCount={}, maxCount={}",
                listId.getId(),
                currentCount,
                MAX_CARDS_PER_LIST
            );
