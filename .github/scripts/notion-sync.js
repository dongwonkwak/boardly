...
class GitHubNotionSync {
  constructor() {
    ...
    this.mappingFile = path.join(this.workspace, '.github/scripts/page-mapping.json');
  ...
  savePageMapping() {
    try {
      ...
      this.pageMapping.lastSync = new Date().toISOString();
      fs.writeFileSync(this.mappingFile, JSON.stringify(this.pageMapping, null, 2));
  ...
  async createPage(relativePath, title, content, fileHash) {
    ...
    try {
      ...
      // 매핑 정보 업데이트
      this.pageMapping.mappings[relativePath] = response.id;
      this.pageMapping.hashes[relativePath] = fileHash;
  ...
  async syncFile(relativePath) {
    ...
    const existingPageId = this.pageMapping.mappings?.[relativePath];
    const lastHash = this.pageMapping.hashes?.[relativePath];
    ...
    try {
      ...
      if (process.env.FORCE_SYNC === 'true') {
        ...
        unmappedFiles = allFiles.filter(file => 
          !this.pageMapping.mappings[file] && !filesToSync.includes(file)
        );
