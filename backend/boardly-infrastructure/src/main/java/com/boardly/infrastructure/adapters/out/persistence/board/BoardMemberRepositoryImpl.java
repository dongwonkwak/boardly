...
public class BoardMemberRepositoryImpl implements BoardMemberRepository {
    ...
    public Either<Failure, BoardMember> save(BoardMember boardMember) {
        try {
            BoardMemberEntity entity = BoardMemberEntity.fromDomainEntity(
                boardMember
            );
            BoardMemberEntity savedEntity = boardMemberJpaRepository.save(
                entity
            );
            log.debug(
                "보드 멤버 저장 완료: memberId={}, boardId={}, userId={}",
    ...
    public Optional<BoardMember> findById(BoardMemberId memberId) {
        ...
        return boardMemberJpaRepository
            .findById(memberId.getId())
            .map(BoardMemberEntity::toDomainEntity);
    }

    @Override
    public Optional<BoardMember> findByBoardIdAndUserId(
        BoardId boardId,
        UserId userId
    ) {
        log.debug(
        ...
        return boardMemberJpaRepository
            .findByBoardIdAndUserId(boardId.getId(), userId.getId())
            .map(BoardMemberEntity::toDomainEntity);
    }
    ...
    public List<BoardMember> findActiveByBoardId(BoardId boardId) {
        ...
        return boardMemberJpaRepository
            .findByBoardIdAndIsActiveTrue(boardId.getId())
            .stream()
            .map(BoardMemberEntity::toDomainEntity)
            .toList();
    }
    ...
    @Override
    public List<BoardMember> findByBoardIdAndRole(
        BoardId boardId,
        BoardRole role
    ) {
        log.debug(
            "findByBoardIdAndRole: boardId={}, role={}",
            boardId.getId(),
            role
        );
        return boardMemberJpaRepository
            .findByBoardIdAndRole(boardId.getId(), role)
            .stream()
            .map(BoardMemberEntity::toDomainEntity)
            .toList();
    }
    ...
    public Either<Failure, Void> delete(BoardMemberId memberId) {
        try {
            if (boardMemberJpaRepository.existsById(memberId.getId())) {
                ...
                return Either.left(
                    Failure.ofNotFound("BOARD_MEMBER_NOT_FOUND")
                );
            }
        } catch (Exception e) {
    ...
    public boolean existsByBoardIdAndUserId(BoardId boardId, UserId userId) {
        return boardMemberJpaRepository.existsByBoardIdAndUserId(
            boardId.getId(),
            userId.getId()
        );
    }
    ...
    public long countActiveByBoardId(BoardId boardId) {
        ...
        return boardMemberJpaRepository.countByBoardIdAndIsActiveTrue(
            boardId.getId()
        );
    }
    ...
    public Either<Failure, Void> deleteByBoardId(BoardId boardId) {
        try {
            log.debug(
                "보드의 모든 멤버 삭제 시작: boardId={}",
                boardId.getId()
            );
            boardMemberJpaRepository.deleteByBoardId(boardId.getId());
            log.debug(
                "보드의 모든 멤버 삭제 완료: boardId={}",
                boardId.getId()
            );
            return Either.right(null);
        } catch (Exception e) {
            log.error(
                "보드의 멤버 삭제 실패: boardId={}, error={}",
                boardId.getId(),
                e.getMessage(),
                e
            );
            return Either.left(
                Failure.ofInternalServerError(
                    "보드의 멤버 삭제에 실패했습니다: " + e.getMessage()
                )
            );
        }
    }
