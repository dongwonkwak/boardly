package com.boardly.features.boardlist.infrastructure.policy;
...
public class BoardListCreationPolicy {
    ...
    public Either<Failure, Void> canCreateBoardList(BoardId boardId) {
        log.debug(
            "보드 리스트 생성 정책 검증 시작: boardId={}",
            boardId.getId()
        );

        return checkListCountLimit(boardId).peek(v ->
            log.debug(
                "보드 리스트 생성 정책 검증 성공: boardId={}",
                boardId.getId()
            )
        );
    }
    ...
    private Either<Failure, Void> checkListCountLimit(BoardId boardId) {
        ...
        log.debug(
            "현재 리스트 개수 확인: boardId={}, currentCount={}, maxCount={}",
            boardId.getId(),
            currentCount,
            maxLists
        );
        ...
        if (currentCount >= maxLists) {
            log.warn(
                "보드 리스트 개수 제한 초과: boardId={}, currentCount={}, maxCount={}",
                boardId.getId(),
                currentCount,
                maxLists
            );
            return Either.left(
                Failure.ofPermissionDenied(
                    String.format(
                        "보드당 최대 %d개의 리스트만 생성할 수 있습니다. (현재: %d개)",
                        maxLists,
                        currentCount
                    )
                )
    ...
    public enum ListCountStatus {
        ...
        ABOVE_RECOMMENDED(
            "권장 초과",
            "권장 개수를 초과했습니다. 리스트를 정리하는 것을 고려해보세요."
        ),
        ...
        LIMIT_REACHED(
            "제한 도달",
            "최대 리스트 개수에 도달했습니다. 새 리스트를 생성하려면 기존 리스트를 삭제해주세요."
        );
