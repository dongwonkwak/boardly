...
public class BoardListMovePolicy {
     ...
     */
    public Either<Failure, Void> canMoveWithinSameBoard(
        BoardList boardList,
        int newPosition
    ) {
        log.debug(
            "같은 보드 내 리스트 이동 정책 검증: listId={}, oldPosition={}, newPosition={}",
        ...
        return validatePosition(newPosition)
            .flatMap(v ->
                validatePositionRange(boardList.getBoardId(), newPosition)
            )
    ...
    private Either<Failure, Void> validatePositionRange(
        ...
        if (position > listCount) {
            log.warn(
                "위치 범위 초과: boardId={}, position={}, listCount={}",
                boardId.getId(),
                position,
                listCount
            );
