...
class CardControllerSearchCardsTest {
    ...
    private static final String TEST_SEARCH_TERM = "테스트";
    ...
    @Test
    @DisplayName("카드 검색 성공 시 200 응답을 반환해야 한다")
    void searchCards_withValidRequest_shouldReturn200() {
        // given
        ...
        // when
        ResponseEntity<?> response = cardController.searchCards(TEST_LIST_ID, TEST_SEARCH_TERM, httpRequest, jwt);
        ...
        verify(cardQueryUseCase).searchCards(any(ListId.class), any(String.class), any(UserId.class));
    }

    @Test
    @DisplayName("카드 검색 실패 시 failureHandler가 호출되어야 한다")
    void searchCards_withFailure_shouldCallFailureHandler() {
        // given
    ...
    @Test
    @DisplayName("검색 결과가 없는 경우 빈 리스트를 반환해야 한다")
    void searchCards_withNoResults_shouldReturnEmptyList() {
        // given
    ...
    void searchCards_shouldExtractUserIdFromJwt() {
        // given
        List<Card> cards = createTestCards();
        ...
        // when
        cardController.searchCards(TEST_LIST_ID, TEST_SEARCH_TERM, httpRequest, jwt);
