...
class GetUserServiceTest {
    ...
    @BeforeEach
    void setUp() {
        getUserService = new GetUserService(userFinder, messageResolver);
    }
    ...
    void get_withExistingUserId_shouldReturnUser() {
        ...
        when(userFinder.findUserOrThrow(userId)).thenReturn(expectedUser);
        ...
        verify(messageResolver, never()).getMessage(anyString());
    }
    ...
    void get_withUserNotFound_shouldReturnNotFound() {
        ...
        when(userFinder.findUserOrThrow(userId)).thenThrow(new UsernameNotFoundException(userId.getId()));
        when(messageResolver.getMessage("validation.user.email.not.found")).thenReturn(errorMessage);
    ...
    void get_withSameUserIdMultipleTimes_shouldCallFinderEachTime() {
        ...
        // 캐싱은 UserFinderImpl에서 설정되지만, 서비스는 매번 위임
