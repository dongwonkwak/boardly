...
public class DashboardService implements GetDashboardUseCase {
    ...
    public Either<Failure, DashboardResponse> getDashboard(GetDashboardCommand command) {
        ...
        // 2. 사용자 존재 확인
        if (!userFinder.userExists(command.userId())) {
        ...
        return Try.of(() -> {
            ...
            Either<Failure, List<Board>> boardsResult = boardQueryService.getUserBoards(
                    GetUserBoardsQuery.activeOnly(command.userId()));
     ...
     * 사용자의 보드 역할을 조회합니다.
     */
    private String getUserBoardRole(BoardId boardId, com.boardly.shared.common.value.UserId userId) {
        try {
