...
class CardValidatorTest {
    ...
    class CreateCardValidationTest {
        ...
        void validateCreate_WithValidCommand_ShouldReturnSuccess() {
            ...
            when(commonValidationRules.cardTitleComplete(any())).thenReturn(
                Validator.valid()
            );
            when(
                commonValidationRules.cardDescriptionComplete(any())
            ).thenReturn(Validator.valid());
            when(commonValidationRules.listIdRequired(any())).thenReturn(
                Validator.valid()
            );
            ...
            // when
            ValidationResult<CreateCardCommand> result =
                cardValidator.validateCreate(command);
        ...
        void validateCreate_WithInvalidTitle_ShouldReturnFailure() {
            ...
            when(commonValidationRules.cardTitleComplete(any())).thenReturn(
                Validator.invalid("title", "제목은 필수입니다")
            );
            when(
                commonValidationRules.cardDescriptionComplete(any())
            ).thenReturn(Validator.valid());
            ...
            assertThat(violation.getMessage()).isEqualTo("제목은 필수입니다");
        }
        ...
        void validateCreate_WithMultipleFailures_ShouldReturnAllErrors() {
            ...
            when(
                ...
                Validator.invalid("description", "설명이 너무 깁니다")
            );
            when(commonValidationRules.listIdRequired(any())).thenReturn(
                Validator.invalid("listId", "리스트 ID는 필수입니다")
            );
            when(commonValidationRules.userIdRequired(any())).thenReturn(
                Validator.invalid("userId", "사용자 ID는 필수입니다")
            );
    ...
    class UpdateCardValidationTest {
        ...
        void validateUpdate_WithValidCommand_ShouldReturnSuccess() {
            ...
            // when
            ValidationResult<UpdateCardCommand> result =
                cardValidator.validateUpdate(command);
    ...
    class DeleteCardValidationTest {
        ...
        void validateDelete_WithValidCommand_ShouldReturnSuccess() {
            ...
            // when
            ValidationResult<DeleteCardCommand> result =
                cardValidator.validateDelete(command);
    ...
    class MoveCardValidationTest {
        ...
        void validateMove_WithValidCommand_ShouldReturnSuccess() {
            ...
            // when
            ValidationResult<MoveCardCommand> result =
                cardValidator.validateMove(command);
        ...
        void validateMove_WithInvalidPosition_ShouldReturnFailure() {
            ...
            when(
                messageResolver.getMessage("validation.position.required")
            ...
            assertThat(result.getErrors()).hasSize(1);
            FieldViolation violation = result.getErrors().get(0);
            assertThat(violation.getField()).isEqualTo("newPosition");
            assertThat(violation.getMessage()).isEqualTo(
                "위치는 0 이상이어야 합니다"
            );
        }
        ...
        void validateMove_WithNullPosition_ShouldReturnFailure() {
            ...
            FieldViolation violation = result.getErrors().get(0);
            assertThat(violation.getField()).isEqualTo("newPosition");
            assertThat(violation.getMessage()).isEqualTo("위치는 필수입니다");
        }
    ...
    class CloneCardValidationTest {
        ...
        void validateClone_WithValidCommand_ShouldReturnSuccess() {
            ...
            // when
            ValidationResult<CloneCardCommand> result =
                cardValidator.validateClone(command);
        ...
        void validateClone_WithInvalidNewTitle_ShouldReturnFailure() {
            ...
            when(commonValidationRules.cardTitleComplete(any())).thenReturn(
                Validator.invalid("newTitle", "새 제목은 필수입니다")
            );
            when(commonValidationRules.userIdRequired(any())).thenReturn(
                Validator.valid()
            );
            ...
            assertThat(result.getErrors()).hasSize(1);
            FieldViolation violation = result.getErrors().get(0);
            assertThat(violation.getField()).isEqualTo("newTitle");
            assertThat(violation.getMessage()).isEqualTo(
                "새 제목은 필수입니다"
            );
        }
    ...
    class IntegrationValidationTest {
        ...
        void allValidationMethods_ShouldWorkCorrectly() {
            ...
            // when & then
            assertThat(
                cardValidator.validateCreate(createCommand).isValid()
            ).isTrue();
            assertThat(
                cardValidator.validateUpdate(updateCommand).isValid()
            ).isTrue();
            assertThat(
                cardValidator.validateDelete(deleteCommand).isValid()
            ).isTrue();
            assertThat(
                cardValidator.validateMove(moveCommand).isValid()
            ).isTrue();
            assertThat(
                cardValidator.validateClone(cloneCommand).isValid()
            ).isTrue();
        }
