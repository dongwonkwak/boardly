...
@Repository
public interface BoardMemberJpaRepository
    extends JpaRepository<BoardMemberEntity, String> {
    /**
     * 보드 ID와 사용자 ID로 보드 멤버를 조회합니다.
     */
    Optional<BoardMemberEntity> findByBoardIdAndUserId(
        String boardId,
        String userId
    );
     ...
     */
    List<BoardMemberEntity> findByBoardIdAndRole(
        String boardId,
        BoardRole role
    );
     ...
     */
    Optional<BoardMemberEntity> findByBoardIdAndUserIdAndIsActiveTrue(
        String boardId,
        String userId
    );
